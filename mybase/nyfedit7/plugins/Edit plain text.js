...
try{
	...
	if(xNyf.isOpen()){
		...
		var _is_text_file=function(sExt){
			return (sExt||'').search(/^(html|htm|qrich|txt|ini|log|csv|css|js|xml|pl|php|asp|c|h|cpp|hpp|cxx|hxx|pas|cs|java|py|rb|go|erl|asm|sql|bat|sh|bas|m|mm|swift|md)$/i)==0;
		};
			...
			if(bProceed){
				...
				if(sDocDispName){
					...
					sTxt=textbox({
						...
						, nWidth: 95
						, nHeight: 80
						, sBtnOK: bRO ? '' : _lc('p.Common.Apply', 'Apply')
					});
