...
try{
	...
	if(xNyf.isOpen()){
		...
		var xInitFn = new CLocalFile(localStorage.getItem(sCfgKey1)||''); xInitFn.append(_validate_filename(sItemTitle) + '.epub');
		...
		var vFields = [
			{sField: 'savefile', sLabel: _lc2('DstFn', 'Save as ePub file'), sFilter: 'ePub documents (*.epub)|*.epub|All files (*.*)|*.*', sTitle: plugin.getScriptTitle(), sInit: xInitFn+'', bReq: true}
			...
			, {sField: 'lineedit', sLabel: _lc2('Author', 'Author name of the ePub digital book'), sInit: localStorage.getItem(sCfgKey3)||'', bReq: false}
		];
		...
		if(vRes && vRes.length == 3){
			...
			if(sDstFn && sLang!=undefined && sAuthor!=undefined){

				localStorage.setItem(sCfgKey1, (new CLocalFile(sDstFn)).getParent());
				...
				var sTmpDir='';
				{
					var xTmpDir=new CLocalDir(platform.getTempFolder(), 'nyf2epub'); //xTmpDir.append('nyf2epub');
					if(xTmpDir.exists()){
						sTmpDir=xTmpDir.toString();
					}else{
						if((new CLocalDir()).createDirectory(xTmpDir.toString())){
							sTmpDir=xTmpDir.toString();
						}
					}
				}
				...
				if(sTmpDir){
					...
					if(z && z.create(sDstFn)){
						...
						var bHtml=false, sLineDelimiter='\r\n';
								...
								case 2:
									try{
										...
										if(e.toString().indexOf('too much recursion')>=0){
											...
											sMsg += ('\n\n'+_lc2('TooMuchRecursion.Notes', 'Notes: You may want to revise the HTML document and have these HTML tags properly closed. ePub conforms to XHTML 1.1 Specs.'));
										}
									...
									if(plugin.isReservedNoteFn(sDef)){
										...
										if(sHref){
											...
											if(sExt=='rtf'){
												...
												s=s.replace('%MYBASE-EPUB-MAKER%', 'Generated with <a href="http://www.wjjsoft.com/mybase#htmltree" target="_blank">myBase/HtmlTree Maker</a> by <a href="http://www.wjjsoft.com/#htmltree" target="_blank">Wjj Software</a>');
											}else if(sExt=='md'){
								...
								if(xMani){
									...
									for(var i in vInfoItems){
										...
										if(xItem){
											...
											var sExt=new CLocalFile(sHref).getExtension(false).toLowerCase();
