...
//sHint=Import local files as attachments, including accompanying images for HTML if applicable
			...
			if(vFiles && vFiles.length>0){
				...
				for(var i in vFiles){
					...
					if(nBytes>nLargeSize){
						vWarn.push(xFn.getFileName() + ' (' + _scale_file_size(nBytes) + ")");
					}
				}
				...
				if(bProceed){
					...
					var sTmpDir=platform.getTempFolder(), sTmpName='__nyf7_import_files_as_attachments';
