...
//					};
					...
					var _skip_dir=function(xDir){
						if(xDir && vDirsToSkip.indexOf(''+xDir)<0){
							vDirsToSkip.push(''+xDir);
						}
					};
					...
					var _import_files_as_attachments=function(xSsgPath, xDskPath){
						...
						for(var i in v){
							...
							var xDskFn=new CLocalFile(xDskPath.toString()); xDskFn.append(sName);
							var tMod=xDskFn.getModifyTime();

							var sSsgName=sName.replace(/\.qrich\.html$/i, '.qrich'); //rename .qrich.html to .qrich;

							if(bDefNoteFn){
								var xFn=new CLocalFile(sSsgName);
								var sExt=xFn.getCompleteSuffix(false);
								sSsgName=plugin.getDefNoteFn(sExt);
							}

							var xSsgFn=new CLocalFile(xSsgPath.toString(), sSsgName);
							...
							if(bHtml){
								...
								var sNew=importAccompanyingObjsWithinHtml(xNyf, xSsgPath, sHtml, xDskPath, null, function(sObj, sAttr, sUriRes, nBytes){
									var u=sObj;
									if(u.search(/^javascript:/i)<0 && u.search(/^mailto:/i)<0 && u.search(/^\w+?:\/\//i)<0 && u.search(/[\?\*\#]/)<0){
										...
										if(sSubDir.indexOf(xDskFn.getTitle())==0 && sSubDir.search(/[_.]files$/)>0){
											var xSub=new CLocalDir(xDskPath.toString(), sSubDir);
											_skip_dir(xSub);
										}else{
											//2016.4.20 consider skipping all accompanying sub folders; e.g.  ./images/logo.png
											var xSub=new CLocalDir(xDskPath.toString(), sSubDir.replace(/^\.\//, ''));
											_skip_dir(xSub);
										}
									}
									return sUriRes;
								...
								if(sHtml!=sNew){
									var xTmpFn=new CLocalFile(platform.getTempFile('', '', '.tmp')); plugin.deferDeleteFile(xTmpFn);
							...
							var nBytes=xNyf.createFile(xSsgFn, xDskFn);
							if(nBytes>=0){
					...
					var _import_files_as_hyperlinks=function(xSsgPath, xDskPath){
						...
						if(v.length>0){
							...
							for(var i in v){
								...
								var sName=v[i]||'';
								var xDskFn=new CLocalFile(xDskPath); xDskFn.append(sName);

								bContinue=plugin.ctrlProgressBar(xDskFn, 1, true);
								if(!bContinue) return false;
								...
								if(bHtml){
									...
									for(var j in vTags){
										var xSubDir=new CLocalDir(xDskPath.toString(), sTitle+vTags[j]);
										if(xSubDir.exists()){
											_skip_dir(xSubDir);
										}
									}
								...
								var sUri=xNyf.makeFileLinkWithRelativePath(xDskFn.toString());
								var tMod=xDskFn.getModifyTime();
								...
								sHtml+='\n<tr><td align=left><a href="%URI%">%NAME%</a></td><td align=right>%SIZE%</td><td align=center>%DATE%</td></tr>'
									...
									.replace('%NAME%', htmlEncode(sName))
									.replace('%URI%', sUri)
								;
							}

							var sDirLink=xNyf.makeFileLinkWithRelativePath(xDskPath.toString());
							var sDirLabel=xNyf.applyRelativePath(xDskPath.toString());
							...
							sHtml='<html>'
								...
								+ '</head>'
								+ '<body>'
								+ '<div><b>Directory</b>: <a href="%LINK%">%DIR%</a></div>'.replace('%LINK%', sDirLink).replace('%DIR%', htmlEncode(sDirLabel))
								+ '<hr noshade size=1 />'
								+ '<table>'
								+ '<thead><tr>'
							...
							var xSsgFn=new CLocalFile(xSsgPath); xSsgFn.append('index.html'); //plugin.getDefNoteFn()
					...
					var _import_folder=function(sSsgPath, sDskPath, xActOnFile){
						...
						if(vDirsToSkip.indexOf(sDskPath.toString())<0){ //ignore sub-dirs where HTML doc's accompanying images reside;
							...
							if(xNyf.createFolder(xSubItem)){
								...
								xNyf.setFolderHint(xSubItem, xDskPath.getLeafName());
								...
								for(var i in v){
									...
									var xSubDir=new CLocalFile(xDskPath); xSubDir.append(sName);
