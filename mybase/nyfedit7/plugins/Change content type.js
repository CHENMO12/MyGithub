		...
		if(!xNyf.isReadonly()){
			...
			var _makeUniqueSsgFileName=function(sBaseName, sSuffix){
				...
				do{
					...
					if(!xNyf.fileExists(xFn.toString())) break;
					i++;
				}while(1);
				return sName;
			};
			...
			if(vDisp.length>0){
				...
				var vFields = [
					{sField: "combolist", sLabel: _lc2('Descr', 'Please choose one of attachments as default item content or create a new one'), vItems: vDisp, sInit: iInit}
				];
				...
				if(vRes && vRes.length>0){
					...
					if(sSsgName!=sOldNoteFn){
						...
						if(sOldNoteFn){
							...
							if(xNyf.fileExists(xFn)){
								...
								bSucc=xNyf.renameFile(xFn, sBakName);
							}
						}
						...
						if(bSucc){
							if(sSsgName && sNewNoteFn){
								...
								bSucc=xNyf.renameFile(xFn, sNewNoteFn);
							}
						}
					}
					...
					if(bSucc){
						...
						bSucc=plugin.openSsgFileInplace(-1, xFn.toString(), sEditor, '');
					}
