...
//sHint=Capture webpages from Google Chrome and save as info items
//sCategory=
...
//var _percentDecode_000=function(s){
//	var v=new CByteArray();
//	if(s){
//		for(var i=0; i<Math.floor(s.length/2); ++i){
//			var b=parseInt(s.charAt(i*2)+s.charAt(i*2+1), 16) & 0xff;
//			v.append(b);
//		}
//	}
//	return v;
			...
			if(sDir){
				...
				if(xFn0.exists()){
					...
					if(nFiles>0){
						...
						var sNewItem=new CLocalFile(sParentItem, xNyf.getChildEntry(sCurItem)).toString();
													...
													for(var i in vObjs){
														...
														if(o && o.sFn && o.sData){
															...
															if(o.bEncoded){
																...
																if(vBin){
																	var nBytes=xNyf.saveBytes(xSsgFn, vBin, true);
																	if(nBytes>0) nDone++;
																}
															}else{
																var sHtml=o.sData.replace(/\\r\\n/g, '\n').replace(/\\n/g, '\n').replace(/\\t/g, '\t').replace(/\\\\/g, '\\');
																if(sHtml){
																	var nBytes=xNyf.saveUtf8(xSsgFn.toString(), sHtml, true, true);
																	if(nBytes>0) nDone++;
																}
															}
//alert(nDone);
									...
									if(xDskFn.exists()){
										var nBytes=xNyf.createFile(xSsgFn.toString(), xDskFn.toString(), true); xDskFn.remove();
										if(nBytes>0){
											nDone++;
										}
									}
