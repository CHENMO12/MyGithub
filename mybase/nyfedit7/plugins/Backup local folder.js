...
try{
	...
	if(vRes && vRes.length >= 2){
		...
		var sDirSrc=vRes[0], sDirDst=vRes[1], vChk=vRes[2], bSubFolders=vChk[0], bHiddenFolders=vChk[1], bHiddenFiles=vChk[2];
			...
			var _retrieve_subdirs=function(sRoot, sDir, bRecursively, vSubdirs){
				...
				for(var i in v){
					...
					if(!_dir_ignoring(sName)){
						...
						var xSub=new CLocalFile(xDir.toString(), sName);
						if(!plugin.showProgressMsg(sHint + ': ' + xSub.toString(), true)) throw 'User abort!';
						var sChild=new CLocalFile(sDir, sName).toString();
			...
			var _retrieve_fileinfo=function(sRoot, sDir, vFiles){
				...
				for(var i in v){
					...
					if(!_file_ignoring(sName)){
						var xFn=new CLocalFile(xDir.toString(), sName);
						var sHint='[Files: ' + (++nFiles) + ' Elapsed: ' + Math.floor((new Date() - tStart)/1000) + ']';
						if(!plugin.showProgressMsg(sHint + ': ' + xFn.toString(), true)) throw 'User abort!';
						var xRel=new CLocalFile(sDir, sName);
						vFiles.push(xRel.toString());
					}
				}
			};
					...
					var _copy_files=function(vNames, sSrc0, sDst0, sRel0){
						for(var i=0; i<vNames.length; ++i){
							...
							//if(!_file_ignoring(sName))
							{
								...
								var sRel=xRel.toString().replace(/^\.[\/\\]/, '').replace(/[\/\\]\.[\/\\]/, '/');
								...
								if(xFnSrc.exists()){
									...
									if(!plugin.showProgressMsg(sHint + ': ' + xRel.toString(), true)) throw 'User abort!';
									...
									if(xFnSrc.copyTo(xFnDst.getDirectory(), "")){
										...
										vLog.push('File copied: ' + xFnSrc + ' -> ' + xFnDst);
									}else{
										nFails++;
										//vFail.push(xRel.toString());
										vLog.push('*** Failure copying file: ' + xFnSrc + ' -> ' + xFnDst);
									}
								}else{
									nFails++;
									vLog.push('*** File missing: ' + xFnSrc);
									//vFail.push(xRel.toString());
								}
							}
						}
							...
							if(xSrc.exists()){
								if(xDst.create()){
									...
									for(var i=0; i<vSub.length; ++i){
										...
										_copy_folder(sSrc0, sDst0, xSub.toString());
									}
								}else{
									nFails++;
									vLog.push('*** Failure creating folder: ' + xDst);
									//vFail.push(sRel);
								}
							}else{
								nFails++;
								vLog.push('*** Folder missing: ' + xSrc);
								//vFail.push(sRel);
							}
						};
