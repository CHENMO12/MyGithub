		...
		if(!xNyf.isReadonly()){
			...
			if(plugin.getCurNavigationTab()=='Outline'){
				...
				if(vRes && vRes.length>=2){
					...
					sCurItem=bCurBranch ? (plugin.getCurInfoItem()||plugin.getDefRootContainer()) : plugin.getDefRootContainer();
					...
					var _act_on_treeitem=function(sSsgPath, iLevel){
						...
						if(xNyf.folderExists(sSsgPath, false)){
							...
							for(var i in vFiles){
								...
								if(bShortcuts && xNyf.isShortcut(xSsgFn)){
									if(_act_on_shortcut(xNyf, xSsgFn.toString(), _location_info(sSsgPath, sName))){
										//succeeded;
									}
								}else if(bHyperlinks && sExt && vExtsHtml.indexOf(sExt)>=0){
									if(_act_on_htmldoc(xNyf, xSsgFn.toString(), _location_info(sSsgPath, sName))){
										//succeeded;
									}
								}
							}
