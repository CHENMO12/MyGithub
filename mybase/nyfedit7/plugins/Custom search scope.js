...
try{
	...
	if(xNyf.isOpen()){
		...
		var vScope=[
			(localStorage.getItem(sCfgKey3)||'true')+'|'+_lc2('Titles', 'Item titles')
			, (localStorage.getItem(sCfgKey4)||'true')+'|'+_lc2('HtmlContents', 'HTML contents')
			, (localStorage.getItem(sCfgKey5)||'true')+'|'+_lc2('Attachments', 'Attachments')
			];

		var vFields = [
			{sField: "lineedit", sLabel: _lc2('Phrase', 'Search for words or RegExp (eg. /pattern/i ):'), sInit: localStorage.getItem(sCfgKey1)||'', bReq: true}
			, {sField: 'check', sLabel: 'Content scope:', vItems: vScope}
			, {sField: "combolist", sLabel: _lc2('Range', 'Outline scope:'), vItems: vRange, nInit: localStorage.getItem(sCfgKey2)||'', bReq: false}
			, {sField: 'check', sLabel: '', vItems: [localStorage.getItem(sCfgKey6)+'|'+_lc2('AllDBs', 'Within all opened databases')]}
		];
		...
		if(vRes && vRes.length>=4){
			...
			if(bTitles || bContents || bAttachments){
				...
				plugin.beginUpdateResultsList(plugin.getScriptTitle()+'  ['+sFor+']', true);
				...
				for(var iDbPos in mToScan){
					...
					if(vII.length>0){
						...
						for(var i in vII){
							...
							var bDefNote=plugin.isReservedNoteFn(sSsgName); //(sSsgName==sDefNoteFn);
				...
				plugin.endUpdateResultsList(sFindStr, true);
			}
		}
