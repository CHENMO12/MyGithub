...
try{
	...
	if(vRes && vRes.length >0){
		...
		var _retrieve_fileinfo=function(sDir, xFiles, xSizes){
			...
			for(var i in v){
				...
				if(!_file_ignoring(sName)){
					...
					var xFn=new CLocalFile(sDir); xFn.append(sName); plugin.showProgressMsg(sHint + ': ' + xFn.toString(), true);
					var nSize=xFn.getFileSize(), sSize=nSize.toString(16);
					var tMod=xFn.getModifyTime();
					if(xSizes[sSize]){
						xSizes[sSize].push(xFn.toString());
					}else{
						xSizes[sSize]=[xFn.toString()];
					}
					nFiles++;
				}
			}
		...
		var _retrieve_fileinfo_recursively=function(sDir, xFiles, xSizes){

			_retrieve_fileinfo(sDir, xFiles, xSizes);
			...
			for(var i in v){
				...
				if(!_dir_ignoring(sName)){
					...
					_retrieve_fileinfo_recursively(xSubDir.toString(), xFiles, xSizes);
				}
			}
		};
		...
		var _find_duplicates=function(vDirs){
			...
			for(var j in vDirs){
				var sDir=vDirs[j];
				if(sDir){
					_retrieve_fileinfo_recursively(sDir, xFiles, xSizes);
				}
			}
					...
					for(var i in vFn){
						...
						if(sFn){
							...
							if(nSize.toString(16)==j){
								...
								plugin.showProgressMsg(sHint + ': ' + sFn, true);
