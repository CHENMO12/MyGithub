...
//sValidation=nyfjs
//sCaption=Insert column after
//sHint=Insert a column into the table after the current cell
//sCategory=MainMenu.Table; ToolBar.Table
//sPosition=TBL-01-INS-03
//sCondition=CURDB; DBRW; CURINFOITEM; FORMATTED; EDITING; TABLE
//sID=p.Table.InsColAfter
//sAppVerMin=7.0
//sShortcutKey=
//sAuthor=wjjsoft
	...
	if(xNyf.isOpen()){
		...
		if(!xNyf.isReadonly()){
			...
			if(plugin.isContentEditable()){
				//var sCode='insertColumnAfter();';
				//plugin.runDomScript(-1, sCode);
				plugin.insertColumnAfter();
			}else{
