...
try{
	...
	var vOpts=[
		  _lc2('Names', 'Search file names only')
		, _lc2('Content', 'Search file content only')
		, _lc2('Both', 'Search file names & content')
		];

	var vFields = [
		{sField: "lineedit", sLabel: _lc2('Phrase', 'Enter a text string or RegExp (e.g. /pattern/i ) to search for'), sInit: localStorage.getItem(sCfgKey1)||''}
	...
	if(vRes && vRes.length==3){
		...
		var sFor=vRes[0], sDir=vRes[1], iSel=vRes[2];
		...
		if(sFor && sDir && iSel>=0){
			...
			var _get_file_content=function(sPath, sName, sStatus){
				...
				var xDate=xFn.getModifyTime(), sExt=xFn.getExtension(false).toLowerCase();
			...
			var _search_file_content=function(sPath, sName, sStatus){
				var xFn=new CLocalFile(sPath); xFn.append(sName);
				var sExt=xFn.getExtension(false), nLen=xFn.getFileSize();
