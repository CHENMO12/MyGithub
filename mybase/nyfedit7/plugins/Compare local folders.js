...
try{
	...
	var vFields = [
		{sField: "folder", sLabel: _lc2('Dir1', 'Select a folder path to compare'), sInit: localStorage.getItem(sCfgKey1)||''}
	...
	if(vRes && vRes.length >= 2){
		...
		var _retrieve_fileinfo=function(sRoot, sDir, xFiles){
			var xDir=new CLocalDir(sRoot); xDir.append(sDir);
			...
			for(var i in v){
				...
				if(!_file_ignoring(sName)){
					...
					var xFn=new CLocalFile(xDir.toString()); xFn.append(sName);
					...
					var xRel=new CLocalFile(sDir); xRel.append(sName);
					xFiles[xRel.toString()]={sFn: xFn.toString(), tMod: tMod, nSize: nSize};
					nFiles++;
				}
			}
			nDirs++;
			...
			if(bSubFolders){
				...
				for(var i in v){
					...
					if(!_dir_ignoring(sName)){
						var xSubDir=new CLocalDir(sDir); xSubDir.append(sName);
						_retrieve_fileinfo_recursively(sRoot, xSubDir.toString(), xFiles);
					}
				}
			}
		};
		...
		var _compare_existence=function(v1, v2){
			var vDiff=[];
			for(var j in v1){
				if(!plugin.showProgressMsg(j, true)) throw 'User abort!';
				if(!v2[j]){
