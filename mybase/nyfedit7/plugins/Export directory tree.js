	...
	if(xNyf.isOpen()){
		...
		if(sDstDir){
			...
			var _unique_dirname=function(sDir, sName){
				var sRes=sName||'sub';
				var xFn=new CLocalDir(sDir); xFn.append(sRes);
				while(xFn.exists()){
					sRes=sName+'-'+Math.round(Math.random()*1000);
					xFn=new CLocalDir(sDir); xFn.append(sRes);
				}
				return sRes;
			};
			...
			var vDirTrack=[], vFails=[], sDefNoteFn=plugin.getDefNoteFn(), nFolders=0, nFiles=0;
			var _act_on_treeitem=function(sSsgPath, iLevel){
				...
				if(xNyf.entryExists(sSsgPath, false)){
					...
					if(xDir.exists() && xDir.createDirectory(sSub)){
						xDir.append(sSub);
						nFolders++;
						vDirTrack[iLevel+1]=xDir.toString();
						var vFiles=xNyf.listFiles(sSsgPath);
						for(var i in vFiles){
							var sFn=vFiles[i];
							var xSsgFnSrc=new CLocalFile(sSsgPath); xSsgFnSrc.append(sFn);
							//var sFn2=(sFn==sDefNoteFn) ? sRenameDefNote : sFn; sFn2=_unique_filename(xDir, sFn2);
							var sFn2=plugin.isReservedNoteFn(sFn) ? _renameDefNoteFn(sFn) : _renameRichTextFn(sFn); sFn2=_unique_filename(xDir, sFn2);
							var xDst=new CLocalFile(xDir.toString()); xDst.append(sFn2);
							if(xNyf.isShortcut(xSsgFnSrc)){
								var bResolve=true, sDskFn=xNyf.getShortcutFile(xSsgFnSrc.toString(), bResolve);
								var xDskFn=new CLocalFile(sDskFn);
								if(xDskFn.exists() && xDskFn.copyTo(xDir.toString(), sFn2)){
									nFiles++;
								}else{
								...
								if(sExt=='html' || sExt=='htm'){
									var sHtml=xNyf.loadText(xSsgFnSrc.toString(), 'html');
								...
								if(xNyf.exportFile(xSsgFnSrc, xDst)<0){
									vFails[vFails.length]=sFn;
								}else{
									nFiles++;
								}
