			...
			if(plugin.isContentEditable()){
				...
				if(sEditor=='htmledit'){
					...
					if(vRes && vRes.length == 3){
						...
						/*var sCode = 'parCssUtil([{key:"text-indent", val:"%START%"}, {key:"margin-left", val:"%LEFT%"}, {key:"margin-right", val:"%RIGHT%"}]);'.replace(/%START%/, sStart).replace(/%LEFT%/, sLeft).replace(/%RIGHT%/, sRight);
