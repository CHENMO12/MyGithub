	...
	if(xNyf.isOpen()){
		...
		if(!xNyf.isReadonly()){
			...
			if(plugin.isContentEditable()){
				...
				var sInit;
				{
					var sCode = 'parAttrUtil("%sKey%");'.replace(/%sKey%/, 'title');
					var vOld=plugin.runDomScript(-1, sCode);
				...
				if(vRes && vRes.length == 1){
					...
					var sCode = 'parAttrUtil("title", "%sTitle%");'
							.replace(/%sTitle%/, sComment)

						+ 'parCssUtil([{key:"text-decoration", val:"%sStyle%"}]);'
							.replace(/%sStyle%/, (sComment?'underline':''));
