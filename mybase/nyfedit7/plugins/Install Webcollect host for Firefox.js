...
//sValidation=nyfjs
//sCaption=Install Webcollect host for Mozilla Firefox
//sHint=Install Webcollect host for myBase to capture webpages from Mozilla Firefox
//sCategory=MainMenu.Tools
//sCondition=
//sID=p.InstWebcHostFirefox
//sAppVerMin=7.0
//sAuthor=wjjsoft
	...
	if(sNameHost){
		...
		if(xFnHost.exists()){
			...
			if(plugin.watchWebcollectCacheDir(true)){
				...
				var sIdOfExtension='webcollect@wjjsoft.com'; //extension ID defined in manifest.json: applications/gecko/id;

				var sNameManifestTempl='com.wjjsoft.webcollect.firefox.json.0';

				var sUrlWebStore='https://addons.mozilla.org/en-US/firefox/addon/mybase-webcollect/';

				var _reinstall_host=function(){
					return confirm(_lc2('Reinstall', 'Re-installing Webcollect host for Mozilla Firefox. Proceed?'));
				};

				var _done_install_host=function(){
					if(confirm(_lc2('Done', 'Successfully installed Webcollect host for Mozilla Firefox.\n\nIf you have previously not added Webcollect extension into Mozilla Firefox, you will need to add it from AMO.\n\nProceed to AMO now?'))){
						platform.openUrl(sUrlWebStore);
					}
				};

				var _failed_install_host=function(){
					alert(_lc2('FailedHost', 'Failed to install webcollect host for Mozilla Firefox.'));
				};
				...
				if(bLinux || bMac){
					...
					var vDirNativeMsg=[];
					if(bLinux){
						vDirNativeMsg.push({appname: 'Mozilla Firefox', appdir: '.mozilla', subdir: 'native-messaging-hosts'});
					}else if(bMac){
						vDirNativeMsg.push({appname: 'Mozilla Firefox', appdir: 'Library/Application Support/Mozilla', subdir: 'NativeMessagingHosts'});
					}
					...
					for(var i in vDirNativeMsg){
						...
						if(sDirNativeMsg && xDirNativeMsg && xDirNativeMsg.exists()){
							...
							alert(_lc2('FirefoxNotAvail', 'Please be sure to first have Mozilla Firefox properly installed before installing Webcollect host.'));
						}
					}

				}else if(bWin){

					var sNameManifestDst='com.wjjsoft.webcollect.firefox.json'; //2018.4.9 for Win32, use different names for chrome/firefox, which will be installed in the plugin folder;
					...
					var _install_manifest_win32=function(){
						...
						if(xFnManifest0.exists()){
							...
							if(sManifest){
								...
								var bProceed=true;
								if(xFnManifestDst.exists()){
									//bProceed=confirm(_lc2('Reinstall', 'Re-installing Webcollect host for Mozilla Firefox; Proceed?'));
									bProceed=_reinstall_host();
								}
								...
								if(nBytes>0){
									...
									if(0){
										...
										bSucc=platform.registrySetValue('HKCU', 'Software\\Mozilla\\NativeMessagingHosts\\com.wjjsoft.webcollect', '', xFnManifestDst.toString());
										if(!bSucc){
