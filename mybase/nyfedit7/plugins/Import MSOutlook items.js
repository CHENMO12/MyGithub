...
//var _vLog=[];
				var _clean_up=function(){
					...
					if(xImgDir.exists()){
						...
						for(var i in vFiles){
							var xFn=new CLocalFile(xImgDir.toString(), vFiles[i]);
							xFn.remove();
						}
						xImgDir.remove();
					}
...
//_vLog.push('Msg['+j+']='+sSubject);
							...
							if(xNyf.createFolder(xChild)){
								...
								xNyf.setFolderHint(xChild, sSubject); nDone++;

								var vEmbed=[];
								{
									xItem.saveAs(xTmpFn, olHTML);
									if(xTmpFn.exists()){
...
//_vLog.push('----------------ACC------------------- sHtml Size= ' + sHtml.length);
											...
											for(var i in vFiles){
												var xFn=new CLocalFile(xAccDir.toString(), vFiles[i]);
//_vLog.push('Acc['+(i)+']='+vFiles[i]+', nSize='+xFn.getFileSize());
											}
//_vLog.push('----------------SRC-------------------');
											var sNew=importAccompanyingObjsWithinHtml(xNyf, xChild, _remove_base_tag(sHtml), xTmpFn.getDirectory(false), null, function(sObj, sTagName, sUriRes, nBytes){
												var u=sUriRes;
												//2016.5.12 the o:href='cid:...' stands for files in attachments area of messages;
												if(u.search(/^cid:image\d{3}.(png|gif|jpg|png)@[\w\.\-]+$/i)==0){
													...
													u=u.replace(/^cid:(image\d{3}\.)(png|gif|jpg|png)@[\w\.\-]+$/i, '$1$2');
													if(u) vEmbed.push(u);
...
//_vLog.push('Src['+(x++)+']='+sObj+' => '+u);
											...
											for(var i in vFiles){
												var xFn=new CLocalFile(xAccDir.toString(), vFiles[i]);
												var xSsg=new CLocalFile(xChild.toString(), vFiles[i]);
												xNyf.createFile(xSsg.toString(), xFn.toString());
//_vLog.push('Acc['+(i)+']='+vFiles[i]+', nSize='+xFn.getFileSize());
											}
										}
									}else{
										xItem.saveAs(xTmpFn, olTxt);
										if(xTmpFn.exists()){
									...
									if(xFiles){
										...
										for(var i=0; i<nFiles; ++i){
											...
											if(xFile && sLeaf){
												...
												xFile.saveAsFile(xTmpFn);
//_vLog.push('Att['+i+']='+sLeaf+', nSize='+xTmpFn.getFileSize());

												if(xTmpFn.exists()){
													xSsgFn=new CLocalFile(xChild, sLeaf);
													if(!xNyf.fileExists(xSsgFn.toString())){ //|| xNyf.getFileSize(xSsgFn.toString())!=xTmpFn.getFileSize()
														sLeaf=getUniqueSsgFileName(xNyf, xChild.toString(), sLeaf);
														if(sLeaf){
															xSsgFn=new CLocalFile(xChild.toString(), sLeaf);
															xNyf.createFile(xSsgFn, xTmpFn);
														}
													}
												}
											}
