			...
			if(plugin.isContentEditable()){
				...
				if(sTxt){
					...
					for(var i=0; i<v0.length; ++i){
						var f=v0[i];
						vFields.push({sField: "lineedit", sLabel: f.sKey, sInit: f.sVal, bRequired: false});
					}

					if(vFields.length>=0){

						vFields.push({sField: "lineedit", sLabel: _lc2('NewFields', '* Add new fields *'), vItems: [], sInit: '', bRequired: ( (vFields.length==0) ? true : false) });
