			...
			if(vFiles && vFiles.length>0){
				...
				for(var j in vFiles){
					...
					if(sSsgPath && sSsgName){
						...
						if(iPos>=0 && iPos<vVisible.length-1){
							//2014.1.14 The next file may be a trashed entry;
							var sNext=vVisible[iPos+1];
							var xSsgFn=new CLocalFile(sSsgPath); xSsgFn.append(sNext);
							...
							if(iPos>=0){
								...
								if(iPos>=0){
									...
									alert('Failed to move the attachment down.');
								}
							}
						}
					}
