...

{
	...
	var nBkColorItem=clWhite, nFgColorItem=clBlack; //for regular items;
	var nBkColorJoint=clBlue; //for joint between items and child items;
	var nAlpha=50;
...
try{
	...
	if(xNyf.isOpen()){
		...
		var xInitFn = new CLocalFile(localStorage.getItem(sCfgKey1)||''); //xInitFn.append(_validate_filename(sItemTitle) + '.png');
		...
		var vFields = [
			{sField: 'savefile', sLabel: _lc2('DstFn', 'Save as image file'), sFilter: 'PNG images (*.png);;JPEG images (*.jpg);;Bitmap (*.bmp);;All files (*.*)|*.*', sTitle: plugin.getScriptTitle(), sInit: xInitFn+'', bReq: true}
		...
		if(vRes && vRes.length==4){
			...
			if(sFnDst && iDirection>=0 && iLineStyle>=0){
				...
				var _init_items=function(sSsgPath, iLevel){
					...
					if((new CLocalFile(sSsgPath)).equals(plugin.getDefRootContainer())) sTitle = (new CLocalFile(xNyf+'')).getTitle();
					...
					var v=c.drawText(sTitle, 0, 0, 0, 0, 'SINGLELINE|CALCRECT'); //CALC-RECT; Be sure to exclude VCENTER;
					if(iLevel == 0){
						v = _size_for_root(v);
					}
				...
				var vLeft=[], vRight=[];
				{
					...
					for(var i in v){
						...
						if(i<nRight) vRight[vRight.length]=xSsgPath+'';
						else vLeft[vLeft.length]=xSsgPath+'';
					}
				}
				...
				c.setViewportOrg(-nCanvasWidthLeft+nPaddingCanvas, nPaddingCanvas+nMarginBottom/2);
				...
				var _draw_line=function(sSsgPath, iLevel){
					...
					if(xNyf.folderExists(sSsgPath, false)){
						...
						if(xItem){
							...
							c.setPen('SOLID', nWidthFrames, nFgColorFrames); c.setPenColor(nFgColorFrames, nAlpha);
							c.setBrushColor(nBkColorItem, nAlpha);
							if(iLevel == 0){
								//var vRect = [xItem.xStart+nFixHori, xItem.yStart, xItem.xStart+xItem.nWidth+nFixHori, xItem.yStart+xItem.nHeight];
								//vRect = _rect_for_root(vRect);
								//c.drawRoundedRect(xItem.xStart+nFixHori, xItem.yStart, xItem.xStart+xItem.nWidth+nFixHori, xItem.yStart+xItem.nHeight, 8, 8);
								c.drawRhombus(xItem.xStart+nFixHori, xItem.yStart, xItem.xStart+xItem.nWidth+nFixHori, xItem.yStart+xItem.nHeight);
							}else{
								c.drawRoundedRect(xItem.xStart+nFixHori, xItem.yStart, xItem.xStart+xItem.nWidth+nFixHori, xItem.yStart+xItem.nHeight, 8, 8);
							}
							...
							if(xOwner){
								...
								c.setPen('SOLID', nWidthLines, nFgColorLines); c.setPenColor(nFgColorLines, nAlpha);
									...
									case 0:
										...
										if(bLeftSide){
											...
											if(yTo>yFrom){
												...
												c.arc(x1, y1, x2, y2, 270, 90);
											}else if(yTo<yFrom){
												//c.arc(x1, y1, x2, y2, xFrom, yFrom, xTo, yTo);
												c.arc(x1, y1, x2, y2, 0, 90);
											}else{
												c.moveTo(xFrom, yFrom);
												c.lineTo(xTo, yTo);
											}
											...
											if(yTo>yFrom){
												//yTo--;
												//c.arc(x1, y1, x2, y2, xFrom, yFrom, xTo, yTo);
												c.arc(x1, y1, x2, y2, 180, 90);
											}else if(yTo<yFrom){
												//c.arc(x1, y1, x2, y2, xTo, yTo, xFrom, yFrom);
												c.arc(x1, y1, x2, y2, 90, 90);
											}else{
												c.moveTo(xFrom, yFrom);
												c.lineTo(xTo, yTo);
											}
				...
				var _draw_item=function(sSsgPath, iLevel){
					...
					if(xNyf.folderExists(sSsgPath, false)){
						...
						if(xItem){
							...
							var nLeft = 0;
							if(iLevel == 0){
								var v = c.drawText(xItem.sTitle, 0, 0, 0, 0, 'SINGLELINE|CALCRECT');
								if(xItem.iIcon>=0) nLeft = (xItem.nWidth - v[0] - nIconWidth - nIconMargin*2) / 2;
								else nLeft = (xItem.nWidth - v[0]) / 2;
							}
							x+=nLeft;

							c.drawText(xItem.sTitle, x+nPaddingLeft, y, xItem.nWidth, xItem.nHeight, 'LEFT|VCENTER|SINGLELINE');
							...
							if(xItem.iIcon>=0){
								...
								if(sIconFn){
									...
									c.drawImage(sIconFn, x, y, nIconWidth, nIconHeight, true);
								}
							}
