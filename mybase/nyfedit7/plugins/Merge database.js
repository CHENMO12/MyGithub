			...
			if(sDbSrc){
				...
				if(xDbSrc.isOpen()){
					...
					var _push_to_absent = function(sSsgPath){
						if(sSsgPath && vAbsent.indexOf(sSsgPath) < 0){
					...
					var _compare_branch = function(sSsgPath){
						...
						for(var i in vFiles){
							...
							if(xDbDst.fileExists(xFn.toString())){
								_compare_ssg_file(xFn.toString());
							}else{
								_push_to_new(xFn.toString());
							}
						}
						...
						if(xDbSrc.folderExists(sSsgPath) && xDbDst.folderExists(sSsgPath)){
							//first list folders in the source db, and test them in the destination db;
							{
								...
								for(var i in vFolders){
									...
									if(xDbDst.folderExists(xSub.toString())){
										_compare_branch(xSub.toString());
									}else{
										_push_to_new(xSub.toString());
									}
								}
							}
							...
							//the list entries in the destination db, and test them in the source db;
							{
								...
								for(var i in vDst){
									...
									if(!xDbSrc.entryExists(xSub.toString())){
										_push_to_absent(xSub.toString());
									}
								}
							}
						}
					...
					var _get_display_text = function(xDb, sSsgPath, sNotes){
						...
						if(bFile){
							...
							//if(sSsgName==plugin.getDefNoteFn()){
						...
						if(sType && sSsgPath){
							...
							var bRoot=(sSsgPath == plugin.getDefRootContainer());
							if(bRoot){
								if(sSsgName){
									var sExt=new CLocalFile(sSsgName).getExtension(false);
									if('bmp;png;jpg'.split(';').indexOf(sExt.toLowerCase())>=0){
										sType='Custom icon';
									}
								}
							}
							...
							if(sSsgName){
								...
								var xTmp=new CLocalFile(sSsgPath); xTmp.append(sSsgName);
								var sHint=xDb.getEntryHint(xTmp.toString(), '');
								if(sHint) sDisp += ' (' + sHint + ')';
							}

							var tMod=xDb.getModifyTime(xFn.toString());
							if(bFile){
								var nSize=xDb.getFileSize(xFn.toString(), 0)
					...
					if(bContinue){
						...
						if(bContinue){
							...
							if(vRes && vRes.length > 0){
									...
									if(xDbSrc.folderExists(sSsgPath)){
										if(xDbDst.createFolder(sSsgPath)){
											...
											for(var i in vFolders){
												var sName = vFolders[i];
												var xSub = new CLocalFile(sSsgPath); xSub.append(sName);
												_copy_ssg_folder(xSub.toString());
											}
								...
								{
									//var tNow = new Date();
									//var xFn = new CLocalFile(plugin.getDefRootContainer()); xFn.append(_NYF7_SSGFN_SYNCINFO);
									//xDbDst.saveUtf8('' + xFn, '' + tNow.getTime(), false, true);
								}
