...
//sCategory=MainMenu.Edit; Context.HtmlEdit; Context.HtmlView; Context.RichEdit; Context.RichView; Context.TextEdit; Context.TextView; Context.Action.Edit;
			...
			if(sCurDocPath && sCurDocName){
				...
				var vRev=[], vDisp=[];
				{
					...
					var _compare_timestamp=function(x, y){
						...
						var xFn=new CLocalFile(sCurDocPath); xFn.append(x);
						var tx=xNyf.getModifyTime(xFn);

						xFn=new CLocalFile(sCurDocPath); xFn.append(y);
						var ty=xNyf.getModifyTime(xFn);
					...
					for(var i in vRev){
						var sFn=vRev[i];
						if(sFn){
							var xFn=new CLocalFile(sCurDocPath); xFn.append(sFn);
							var tMod=xNyf.getModifyTime(xFn), nLen=xNyf.getFileSize(xFn);
				...
				if(vDisp.length>0){
					...
					var vFields = [
						{sField: "combolist", sLabel: _lc2('Descr', 'Please select one of the revisions to be set as current version'), vItems: vDisp, sInit: 0}
					];
					...
					if(vRes && vRes.length>0){
						...
						if(vRev[iSel]!=sCurDocName){
							var xFn=new CLocalFile(sCurDocPath); xFn.append(sCurDocName);

							if(xNyf.fileExists(xFn)){
								xNyf.trashEntry(xFn, false);
							}

							var sNewName=xNyf.undeleteEntry(sCurDocPath, vRev[iSel]);
							if(sNewName){
								plugin.refreshDocViews(-1, xFnCurDoc.toString());
							}
						}
					}
				}else{
