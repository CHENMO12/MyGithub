		...
		if(!xNyf.isReadonly()){
			...
			if(plugin.isContentEditable()){
				...
				var vFields = [
					...
					, {sField: 'lineedit', sLabel: _lc2('Label', 'Label text to display'), sInit: sLabel0||'', bReq: false}
					, {sField: 'lineedit', sLabel: _lc2('Title', 'Title text (optional)'), sInit: sTitle0||'', bReq: false}
					];
				...
				if(vRes && vRes.length==3){
					...
					if(sUrl){
						...
						var sHtml='<a href="' + sUrl + '" title="'+_html_encode(sTitle)+'">' + _html_encode(sLabel) + '</a>';
						plugin.replaceSelectedText(-1, sHtml, true);

					}
				}

				/*var sCfgKey='InsertHyperlink.URL';
				var sMsg=_lc2('Url', 'Enter a hyperlink (URL) to be inserted;');
				var sUrl=prompt(sMsg, localStorage.getItem(sCfgKey)||'http://'); sUrl=_trim(sUrl);
				if(sUrl){
					localStorage.setItem(sCfgKey, sUrl);

					sCfgKey='InsertHyperlink.Text';
					sMsg=_lc2('Text', 'Enter a label text for the hyperlink (URL);');
					var sLabel=prompt(sMsg, localStorage.getItem(sCfgKey)||sUrl||'')||sUrl;
					if(sLabel){
						localStorage.setItem(sCfgKey, sLabel);
						...
						var sHtml='<a href="' + sUrl + '">' + _html_encode(sLabel) + '</a>';
						plugin.replaceSelectedText(-1, sHtml, true);
					}
				}*/
			}else{
