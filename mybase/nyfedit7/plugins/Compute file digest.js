...
try{
	...
	var sFilter='All files (*.*);;Archives(*.zip;*.rar;*.bz2;*.tar;*.gz);;Documents (*.htm;*.html;*.pdf;*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx;*.pages;*.rtf;*.txt);;Executables (*.exe;*.dll;*.dylib;*.so);;Images (*.png;*.jpg;*.gif;*.bmp)';
	...
	if(vRes && vRes.length >= 2){
		...
		if(vFiles && vFiles.length>0){
			...
			for(var i in vFiles){
				...
				var bContinue=plugin.ctrlProgressBar(xFn.toString(), 1, true);
				if(!bContinue) break;

				var sHash=xFn.digest(sAlg);
				if(sHash){
					if(xRes[sHash]){
						xRes[sHash].push(xFn.toString());
					}else{
						xRes[sHash]=[xFn.toString()];
					}
				}
			}
				...
				textbox(
					{
						...
						, sDescr: _lc2('Result', 'List of digest values of the selected files') + ' (' + vAlgs[iAlg] + ')'
						, sDefTxt: sInfo
						, bReadonly: true
						, bWordwrap: false
						, nWidth: 80
