...
class TestNodeAddsecondpartner(BaseCase):
    ...
    def test_正测_成功添加二级合作方(self, depend_nodeInfo):

        with allure.step('准备用例入参'):
            partnerName = testdata.random_node_secondPartnerName()
    ...
    @allure.severity(allure.severity_level.NORMAL)
    def test_正测_成功添加二级合作方_名称在另一个一级合作方下已使用(self, depend_nodeInfo):

        with allure.step('准备用例入参'):
            firstPartnerName = testdata.random_node_secondPartnerName()
            secondPartnerName = depend_nodeInfo["secondPartnerData"]["name"]

        with allure.step('接口请求'):

            res_info = api.bns_node_addFirstPartner(
				partnerName=firstPartnerName,
            )
            partner_areaCode = JsonHelper.parseJson_by_objectpath(res_info, "$.response_data.data.areaCode")
        with allure.step('接口请求'):

            res_info = api.bns_node_addSecondPartner(

                parentAreaCode=partner_areaCode,
    ...
    @allure.severity(allure.severity_level.NORMAL)
    def test_正测_失败添加二级合作方_节点名称已使用(self, depend_nodeInfo):

        with allure.step('接口请求'):

            res_info = api.bns_node_addSecondPartner(
                parentAreaCode=depend_nodeInfo["firstPartnerData"]["areaCode"],
                partnerName=depend_nodeInfo["secondPartnerData"]["name"],
            )
