...
def depend_nodeInfo():
    ...
    with allure.step("前置条件: 添加一级合作方"):
        info = api.scn_node_addFirstPartner()
        
    yield info
    
    with allure.step("清理前置条件: 删除一级合作方"):
        api.bns_node_delete(areaCode=info["areaCode"])

@allure.feature("节点管理")
@allure.story("添加一级合作方")
class TestNodeAddfirstpartner(BaseCase):
    ...
    @allure.severity(allure.severity_level.NORMAL)
    def test_正测_失败添加一级合作方_节点名称已使用(self, depend_nodeInfo):

        with allure.step('接口请求'):

            res_info = api.bns_node_addFirstPartner(
                partnerName=depend_nodeInfo["name"],
            )
