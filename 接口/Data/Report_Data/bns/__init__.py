...
class BaseApi():
    ...
    class ContentType(Enum):
        ...
        FILE = "multipart/form-Report_Data"

    def base_request(self, request_method, request_type, request_url, request_data=None, auth=None, headers=None, cookies=None):
...
class TemplateApi():
    ...
    def __init__(self, yaml_file, module_key, func_key):
        ...
        # 检查yaml中是否有以下key："url", "method", "contentType", "Report_Data"
        expect_dictKey = ["desc", "url", "method", "contentType", "Report_Data"]
        validator.check_existKey_dict(yamlInfo_to_dict, *expect_dictKey)

        # 解析yaml信息
        self.http_params = yamlInfo_to_dict["Report_Data"] if yamlInfo_to_dict["Report_Data"] else {}
    ...
    def _parseYaml_params_parameterize(self, http_params, module_key):
        ...
        if http_params:
            for param in http_params.keys():
                parameterize_func_list.append("if {0} is None: {0} = gen_bnsData.random_{1}_{0}()".format(param, module_key))
...
class BnsApi(BusinessApi):
    ...
    def bns_{1}_{2}(self, headers=None, {3}):
        ...
        http_data = api_info["Report_Data"]
        
        # 请求入参
        Report_Data = {{
            {4}
        }}
        Report_Data = self.base_filter_data(Report_Data)
...
class ScnApi(BnsApi):
    ...
    def default_filepath_bns(self):
        file_path = PROJECT_ROOT + os.sep + "output" + os.sep + "template" + os.sep + "bns_api_{}.py".format(self.module_key)
        return  file_path

    @property
    def default_filepath_scn(self):
        file_path = PROJECT_ROOT + os.sep + "output" + os.sep + "template" + os.sep + "scn_api_{}.py".format(self.module_key)
        return  file_path
